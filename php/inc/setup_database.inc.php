<?php
/**
 * This file is part of the Searchdb project
 *
 * Copyright (c) 2017, Sochima Biereagu
 * Under MIT License
 */

/* this script creates the  Searchdb database and its' tables if the're not yet created */
...
if ($conn -> connect_error) {
    if ($isCMD) {
        exit(PHP_EOL."Error establishing a MySQL database connection, run ".PHP_EOL." $ ./bin/Searchdb config".PHP_EOL);
    }
  ...
  <html>
    <head>
     ...
     <big>
      <br> <br> <br>
      <fieldset>
       <legend> Searchdb &lt; MySQL Connection Error &gt; </legend>
         Error establishing a MySQL database connection, please setup Searchdb by <code>`cd`</code>ing into Searchdb's directory and run <br>
          <pre>$ ./bin/Searchdb config</pre>
...
<br><br>
<code>cd</code> into Searchdb's directory and run
  <pre>
  <code> $ ./bin/Searchdb config </code>
  </pre>
  to rectify this.
sql
    ); // exit
